...
// Inside the Run() function, you will also have to add an extra argument "pageWidth," which is declared on line 4 when you call the checkPageBounds() function below.
setInterval(() => {
  Run();
}, 300);
...
function checkPageBounds(direction, imgWidth, pos, pageWidth) {
  if (pos + imgWidth > pageWidth && direction === 0) {
    // PacMan is at the right edge and moving right, reverse direction
    return 1;
  } else if (pos < 0 && direction === 1) {
    // PacMan is at the left edge and moving left, reverse direction
    return 0;
  }
  // PacMan is within bounds, maintain current direction
  return direction;
}
